...
def parse_kml_file(filename): # Make this code better and Document it
    ...
    for child in root[0]:
        if child.tag == "{http://www.opengis.net/kml/2.2}Placemark":
            ...
            coords = []
            # coordinates tag
            p_coords = child.find("{http://www.opengis.net/kml/2.2}LineString").find(
            ...
            for line in p_coords.text.split("\n"):
                ...
                parts = line.split(",")
                coords.append(
                    Coordinate(float(parts[0]), float(parts[1]), float(parts[2]))
                )
...
def calc_distance(coords: list[Coordinate], current_coord: Coordinate):
    ...
    for i in range(stop_index + 1):
        if i > 0:
            sum_distance += heversine_and_azimuth(
                coords[i - 1].lon, coords[i - 1].lat, coords[i].lon, coords[i].lat
            )[0]
